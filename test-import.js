...
(async()=>{
  /* Two real products are updated by this suite and put back exactly as
     they were found. It used to "restore" hard-coded prices, which quietly
     overwrote whatever the shop had set since. */
  const REAL = ["SGPK-001", "SGPK-002"];
  const saved = await sql`
    SELECT id, sku, price, sale_price, stock_quantity, status, category_id, subcategory
      FROM products WHERE sku = ANY(${REAL})`;
  const savedShades = await sql`
    SELECT id, stock_quantity FROM product_variants WHERE product_id = ANY(${saved.map((r) => r.id)})`;
  const [{ n: productCount }] = await sql`SELECT count(*)::int n FROM products`;
  const [{ n: listed }] = await sql`SELECT count(*)::int n FROM products WHERE status <> 'ARCHIVED'`;
  t("the real products this suite touches exist", saved.length === 2, saved.map((r) => r.sku).join(", "));

  try {
    // 1. template reflects the live catalogue
    const tpl = await I.template();
    const tplRows = require(P+"/lib/csv.js").parse(tpl);
    t("template includes the current catalogue", tplRows.length === listed + 1, `${tplRows.length-1} products + header`);

    // 2. analyse a file: 1 new, 1 update (by SKU), 1 error, 1 warning.
    //    New rows are drafts, so nothing the suite creates is ever on the shop.
    const file = [
      "Name,SKU,Price,Stock,Category,Subcategory,Status,Shades",
      "Brand New Gloss,CSV-NEW-1,1250,15,lips,Lip Gloss,draft,Pink|Coral",
    ...
    t("apply creates and updates", r.created===3 && r.updated===1, `created=${r.created} updated=${r.updated} skipped=${r.skipped} failed=${JSON.stringify(r.failed)}`);
    ...
    t("price-only update preserves shades", beforeShades===afterShades && beforeShades>0, `${beforeShades} → ${afterShades}`);
  } finally {
    // cleanup: put everything back as it was found
    await sql`DELETE FROM products WHERE sku LIKE 'CSV-%'`;
    for (const p of saved) {
      await sql`
        UPDATE products SET price = ${p.price}, sale_price = ${p.sale_price}, stock_quantity = ${p.stock_quantity},
               status = ${p.status}::product_status, category_id = ${p.category_id}, subcategory = ${p.subcategory}
         WHERE id = ${p.id}`;
    }
    for (const v of savedShades) await sql`UPDATE product_variants SET stock_quantity = ${v.stock_quantity} WHERE id = ${v.id}`;
  }

  const [final]=await sql`SELECT count(*)::int n FROM products`;
  t("cleaned up", final.n===productCount, `${final.n} products`);
  const restored = await sql`SELECT id, price, stock_quantity FROM products WHERE sku = ANY(${REAL})`;
  t("real products restored exactly", restored.every((x) => {
    const was = saved.find((s) => String(s.id) === String(x.id));
    return was && Number(x.price) === Number(was.price) && x.stock_quantity === was.stock_quantity;
  }), restored.map((x) => `#${x.id} Rs.${Number(x.price)} stock ${x.stock_quantity}`).join(", "));
