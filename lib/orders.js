...
async function createOrder(input) {
  ...
  const soldOut = [];
  const taken = [];          // stock already taken, to give back if a later step fails
  try {
    for (const i of priced) {
      /* product_slug and image_url are snapshots too: an order must still
         show what was bought after the product is renamed, archived or
         deleted. */
      await sql`
    ...
    /* These writes are separate statements, not one transaction. If a
       product or shade was deleted between pricing and here, an order line
       fails its foreign key after the order row already exists. Undo by
       hand — give the stock back and remove the order (its lines cascade)
       — so the customer's retry places a real order instead of the
       idempotency key returning an empty one. */
    for (const i of taken) {
      try {
        if (i.variantId) {
          await sql`UPDATE product_variants SET stock_quantity = stock_quantity + ${i.qty} WHERE id = ${i.variantId}`;
          await syncProductStock(i.productId);
        } else {
          await sql`UPDATE products SET stock_quantity = stock_quantity + ${i.qty} WHERE id = ${i.productId}`;
        }
      } catch (undo) { console.error("[orders] could not return stock:", undo.message); }
    }
    try { await sql`DELETE FROM orders WHERE id = ${order.id}`; }
    catch (undo) { console.error("[orders] could not remove the incomplete order:", undo.message); }
    if (/foreign key/i.test(String(e.message))) {
      throw new Error("One of the items is no longer available. Please refresh and try again.");
    }
    throw e;
  }
