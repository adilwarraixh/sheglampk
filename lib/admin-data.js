...
async function setStock({ productId, variantId, quantity }) {
  ...
  if (variantId) {
    ...
    await syncProductStock(v[0].product_id);
  ...
  /* A product sold in shades has no stock of its own to set: its total is
     ...
     with them until the next save. */
  const [shades] = await sql`
    SELECT count(*)::int AS n FROM product_variants WHERE product_id = ${productId} AND is_available`;
  if (shades && shades.n > 0) throw new Error("This product is stocked per shade. Set the stock on each shade instead.");
  const p = await sql`
