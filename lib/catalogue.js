...
async function attach(products) {
  ...
  return products.map((p) => {
    const pi = imgs[p.id] || [];
    // A retired shade stays in the table so old orders resolve; the shop never shows it.
    const pv = (vars[p.id] || []).filter((v) => v.is_available);
    return {
      ...p,
      ...
      inStock: pv.length ? pv.some((v) => v.stock_quantity > 0) : p.stock_quantity > 0,
    };
  });
}
